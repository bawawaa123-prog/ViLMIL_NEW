...
try:
    ...
    try:
        ...
        # 如果simple_tokenizer不可用,使用备用方案
        class _Tokenizer:
            def encode(self, text):
                return clip_tokenize([text])[0].tolist()
    except:
        # 最后的备用方案
        class _Tokenizer:
            def encode(self, text):
...
class ViLa_MIL_Model(nn.Module):
    ...
    def forward(self, x_s, coord_s, x_l, coords_l, label):
        ...
        # M的形状是 [1, N, 1024]，需要squeeze成 [N, 1024] 才能与 compents.squeeze() 拼接
    ...
    def forward_with_attention(self, x_s, coord_s, x_l, coords_l, label):
        """前向传播并返回注意力权重（用于热力图生成）
        
        注意：此方法返回对原始patches的注意力权重，用于生成热力图
        返回的注意力权重维度与输入patches数量一致
        """
        prompts = self.prompt_learner()
        tokenized_prompts = self.prompt_learner.tokenized_prompts
        text_features = self.text_encoder(prompts, tokenized_prompts)
        ...
        M_high = x_l.float()
        compents_high, cross_attn_weights_l = self.cross_attention_1(self.learnable_image_center, M_high, M_high)
        compents_high = self.norm(compents_high + self.learnable_image_center)

        # 与 forward() 保持完全一致：使用 squeeze() 移除所有大小为1的维度
        H = compents.squeeze().float()
        A_V = self.attention_V(H)  
        A_U = self.attention_U(H)  
        A = self.attention_weights(A_V * A_U) 
        ...
        text_features_low = text_features[:self.num_classes]
        # M的形状是 [1, N, 1024]，需要squeeze(0)成 [N, 1024] 才能与 compents.squeeze() 拼接
        image_context = torch.cat((compents.squeeze(), M.squeeze(0)), dim=0)
        text_context_features, _ = self.cross_attention_2(text_features_low.unsqueeze(1), image_context, image_context)
        text_features_low = text_context_features.squeeze() + text_features_low
        ...
        # 从 cross-attention 获取对原始patches的注意力权重
        # cross_attn_weights shape: [batch, num_heads, query_len, key_len]
        # 我们需要的是对 key (patches) 的注意力: [query_len=16, key_len=N_patches]
        # 平均所有heads，然后对16个prototypes取平均，得到每个patch的重要性
        patch_attention_s = cross_attn_weights_s.mean(dim=1).squeeze(0)  # [16, N_s]
        patch_attention_s = patch_attention_s.mean(dim=0)  # [N_s] - 对所有prototypes取平均
        
        patch_attention_l = cross_attn_weights_l.mean(dim=1).squeeze(0)  # [16, N_l]
        patch_attention_l = patch_attention_l.mean(dim=0)  # [N_l] - 对所有prototypes取平均

        # 返回预测结果和对原始patches的注意力权重
