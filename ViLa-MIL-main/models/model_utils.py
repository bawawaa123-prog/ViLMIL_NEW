...
import torch
# from torch.nn.modules.linear import _LinearWithBias  # Deprecated in newer PyTorch versions
...
# for graph construction (nmslib仅用于HIPT,可选依赖)
try:
    import nmslib
    NMSLIB_AVAILABLE = True
except ImportError:
    NMSLIB_AVAILABLE = False
    print("⚠️  nmslib not installed (only needed for HIPT graph construction)")
...
class Hnsw:
    def __init__(self, space='cosinesimil', index_params=None,
        ...
        if not NMSLIB_AVAILABLE:
            raise ImportError("nmslib is required for HIPT. Install: pip install nmslib")
        self.space = space
