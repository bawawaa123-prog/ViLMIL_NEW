...
class isInContourV1(Contour_Checking_fn):
	...
	def __call__(self, pt): 
		return 1 if cv2.pointPolygonTest(self.cont, (float(pt[0]), float(pt[1])), False) >= 0 else 0
...
class isInContourV2(Contour_Checking_fn):
	...
	def __call__(self, pt): 
		return 1 if cv2.pointPolygonTest(self.cont, (float(pt[0]+self.patch_size//2), float(pt[1]+self.patch_size//2)), False) >= 0 else 0
...
class isInContourV3_Easy(Contour_Checking_fn):
	...
	def __call__(self, pt): 
		...
		for points in all_points:
			if cv2.pointPolygonTest(self.cont, (float(points[0]), float(points[1])), False) >= 0:
				return 1
		return 0
