...
class WholeSlideImage(object):
    ...
    def process_contours(self, save_path, patch_level=0, patch_size=256, step_size=256, **kwargs):
        max_level = len(self.level_downsamples) - 1
        if patch_level > max_level:
            requested_level = patch_level
            patch_level = max_level
            print(
                f"[Warn] slide {self.name} requested patch_level={requested_level}, "
                f"but only {len(self.level_downsamples)} levels are available. "
                f"Falling back to patch_level={patch_level}."
            )
        ...
        for idx, cont in enumerate(self.contours_tissue):
            ...
            if len(asset_dict) > 0:
                if init:
                    ...
                    print(f"Created HDF5 file: {save_path_hdf5}")
                    init = False
                else:
                    save_hdf5(save_path_hdf5, asset_dict, mode='a')

        # Set the hdf5_file path so it can be returned and used for stitching
        if os.path.exists(save_path_hdf5):
            self.hdf5_file = save_path_hdf5
            print(f"HDF5 file saved successfully: {save_path_hdf5}")
        else:
            print(f"Warning: HDF5 file was not created: {save_path_hdf5}")
            self.hdf5_file = None
