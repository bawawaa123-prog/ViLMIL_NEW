...
def process_slide_low(
    ...
    """Low-scale branch: read by coords size and save to out_size (default 256)."""
...
def main():
    parser = argparse.ArgumentParser(
        description='Jiangxi low-scale patch generation: read large level-0 windows and resize to 256'
    )
    ...
    parser.add_argument('--out-size', type=int, default=256, help='output patch size (default: 256)')
    ...
    if args.output_root is None:
        output_dir = os.path.join(args.coords_root, f'patch_images_low_256_from_coords_{args.coords_size}')
    else:
        output_dir = args.output_root
    os.makedirs(output_dir, exist_ok=True)

    print('Start low-scale patch generation')
    ...
    print('Low-scale patch generation finished')
