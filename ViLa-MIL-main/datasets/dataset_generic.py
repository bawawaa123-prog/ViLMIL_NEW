...
def _resolve_h5_path(data_dir, slide_id):
	"""Resolve feature h5 by exact slide_id first, then a TCGA-style UUID-stripped fallback."""
	slide_id = str(slide_id)
	exact_path = os.path.join(data_dir, '{}.h5'.format(slide_id))
	if os.path.exists(exact_path):
		return exact_path

	base_slide_id = slide_id.split('.')[0]
	fallback_path = os.path.join(data_dir, '{}.h5'.format(base_slide_id))
...
class Generic_MIL_Dataset(Generic_WSI_Classification_Dataset):
	...
	def _filter_unavailable_slides(self):
		if self.mode != 'transformer':
			return
		if not self.data_dir_s or not self.data_dir_l:
			return

		keep_indices = []
		missing_slides = []

		for idx, slide_id in enumerate(self.slide_data['slide_id'].tolist()):
			try:
				_resolve_h5_path(self.data_dir_s, slide_id)
				_resolve_h5_path(self.data_dir_l, slide_id)
		...
		self.slide_data = self.slide_data.iloc[keep_indices].reset_index(drop=True)
		self.patient_data_prep()
		self.cls_ids_prep()

		print(
			f"[Warn] Filtered out {len(missing_slides)} slides without both scale feature files."
		)
		preview = ', '.join(missing_slides[:5])
		if preview:
			print(f"[Warn] Missing feature slides (first {min(len(missing_slides), 5)}): {preview}")
		if len(self.slide_data) == 0:
			raise RuntimeError("No slides remain after filtering unavailable feature files.")
	...
	def __getitem__(self, idx):
		...
		if not self.use_h5:
			if self.data_dir_s and self.data_dir_l:
				if(self.mode == 'transformer'):
					scale_s = h5py.File(_resolve_h5_path(data_dir_s, slide_id), 'r')
					scale_l = h5py.File(_resolve_h5_path(data_dir_l, slide_id), 'r')
