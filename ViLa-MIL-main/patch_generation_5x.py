...
def parse_existing_coords(slide_save_dir, expected_patch_size):
    existing_coords = set()
    ...
    for fn in os.listdir(slide_save_dir):
        if not fn.endswith('.png'):
            continue
        parts_new = fn.rsplit('_', 2)
        if len(parts_new) >= 3:
            try:
                x = int(parts_new[-2])
                y = int(os.path.splitext(parts_new[-1])[0])
                existing_coords.add((x, y))
                continue
            except Exception:
                pass

        parts_old = fn.rsplit('_', 3)
        if len(parts_old) < 4:
            continue
        try:
            patch_size = int(parts_old[-3])
            x = int(parts_old[-2])
            y = int(os.path.splitext(parts_old[-1])[0])
        except Exception:
            continue
...
def process_slide(slide_name, source_root, coords_dir, output_dir, patch_level, patch_size, skip_existing=False):
    """处理单个 WSI 文件，按 coords 在指定 level 直接裁剪 5x patches。"""
    ...
    existing_coords = parse_existing_coords(slide_save_dir, patch_size) if skip_existing else set()
...
def main():
    parser = argparse.ArgumentParser(description='从 create_patches_fp.py 输出的 coords 裁剪 5x patches')
    ...
    parser.add_argument('--patch-level', type=int, default=2, help='read_region 使用的 WSI level')
    ...
    print(f"开始裁剪 5x patches (patch_size={args.patch_size}, level={args.patch_level})...")
