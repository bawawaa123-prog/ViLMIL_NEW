...
def save_embeddings(model, fname, dataloader, enc_name, overwrite=False):
    ...
    for file_name in file_names:
        for coord in file_name:
            ...
            # 处理文件名格式：slidename_x_y.png
            # 取最后两个部分作为坐标
            if len(coord) >= 2:
                try:
                    x = int(coord[-2])  # 倒数第二个是x坐标
                    y = int(coord[-1])  # 最后一个是y坐标
                    coords.append([x, y])
                except ValueError:
                    print(f"Warning: 无法解析坐标 {coord}, 跳过该文件")
                    continue
...
def create_embeddings(embeddings_dir, enc_name, dataset, batch_size, save_patches=False,
    ...
    # 处理任何dataset（不仅限于TCGA）
    # pool = ThreadPoolExecutor(max_workers=48)
    for wsi_name in tqdm(os.listdir(patch_datasets)):
        dataset_obj = PatchesDataset(os.path.join(patch_datasets, wsi_name), transform=eval_t)
        dataloader = torch.utils.data.DataLoader(dataset_obj, batch_size=batch_size, shuffle=False, num_workers=0)
