...
def build_uuid_name_file_limited(input_csv, output_csv, limit=None):
    """Build temporary no-header csv: case_id, slide_id, label, with optional head limit."""
    df = pd.read_csv(input_csv)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Jiangxi external queue: create tissue masks + coords h5 from single-level TIFF/OME-TIFF'
    )
    parser.add_argument('--source', default='data/jiangxi/wsi', help='WSI root directory')
    ...
    # Coordinate grid size supports 256/512/1024/2048 for flexible dual-scale recipes.
    parser.add_argument('--coord-size', type=int, choices=SUPPORTED_COORD_SIZES, default=None,
                        help='Coordinate size for h5 generation (256/512/1024/2048/4096)')
    parser.add_argument('--patch-size', type=int, choices=SUPPORTED_COORD_SIZES, default=1024,
                        help='Backward-compatible alias of --coord-size')
    parser.add_argument('--step-size', type=int, default=None,
                        help='Coordinate step size (default: same as coord-size)')
    parser.add_argument('--patch-level', type=int, default=0, help='WSI level used for coords')

    parser.add_argument('--preset', default='jiangxi.csv', help='Preset csv under presets/')
    parser.add_argument('--process-list', default=None, help='Optional process list csv path')
    parser.add_argument('--limit', type=int, default=None, help='Optional: only process first N slides from CSV')
...
def main():
    ...
    try:
        ...
        build_uuid_name_file_limited(args.csv, tmp_path, limit=args.limit)
